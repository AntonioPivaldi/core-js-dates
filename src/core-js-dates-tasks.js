 ...
 *
 * @param {string} str - date and time.
 ...
 * 2, 2024 => 29
 */
function getCountDaysInMonth(m, y) {
 ...
 */
function getCountDaysOnPeriod(start, end) {
 ...
 * 1, 2024 => 8
 */
function getCountWeekendsInMonth(m, y) {
  const date = new Date(y, m - 1);
  const monthLength = getCountDaysInMonth(m, y);
  let days = 0;
 ...
 */
function reverseDateStr(dateStr, joint = '-') {
  return dateStr.split(joint).reverse().join(joint);
}

function getWorkSchedule(period, workDays, offDays) {
  const schedule = [];
  const start = new Date(reverseDateStr(period.start));
  const end = new Date(reverseDateStr(period.end));
  const days = getCountDaysOnPeriod(start, end);
  let workCount = 0;
  let offCount = 0;

  for (let i = 0; i < days; i += 1) {
    if (workCount < workDays) {
      schedule.push(reverseDateStr(start.toISOString().split('T')[0]));
      workCount += 1;
    } else if (workCount === workDays && offCount < offDays) {
      offCount += 1;
      if (offCount === offDays) {
        offCount = 0;
        workCount = 0;
